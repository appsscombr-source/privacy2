...
const GATEWAYS = {
  ...
  omegapay: {
    ...
    async getToken(cfg) {
      // Autenticação via headers fixos (x-public-key / x-secret-key), sem token OAuth
      return null;
    },

    async cashin(cfg, amount, webhookUrl, buyer) {
      // A doc da OmegaPay marca name/phone/document como obrigatórios em client,
      // mas por pedido explícito, mandamos com o que tivermos disponível (hoje,
      // só o e-mail da sessão) e deixamos a API responder o que faltar.
      buyer = buyer || {};
      ...
      const res = await fetch("https://app.omegapayments.com.br/api/v1/gateway/pix/receive", {
        ...
        headers: {
          ...
          "x-public-key": cfg.omegapay_public_key,
          "x-secret-key": cfg.omegapay_secret_key,
        },
        body: JSON.stringify(payload),
      });
      ...
      if (!res.ok) {
        ...
        if (Array.isArray(data.details)) {
          ...
          detail = ` | details bruto: ${JSON.stringify(data.details)}`;
        } else if (Object.keys(data).length) {
          detail = ` | resposta bruta: ${JSON.stringify(data)}`;
        }
        const msg = (data.message || "Erro ao gerar cobrança OmegaPay") + detail;
        throw new Error(msg);
      }
      return {
        pix_code: data.pix && data.pix.code,
        qr_code_url: data.pix && data.pix.image,
        identifier, // nosso identifier — a OmegaPay o ecoa em transaction.identifier no webhook
      };
    },
  },
...
const PARSERS = {
  syncpay: function(body) {
    ...
    return {
      paymentId: d.identifier || d.id,
      status: "approved",
      amount: d.final_amount || d.amount,
      payerEmail: d.client && d.client.email,
      payerName: (d.debtor_account && d.debtor_account.name) || (d.client && d.client.name)
    };
  },
  nexuspag: function(body) {
    if (body.event && body.event !== "payment.confirmed") return null;
    // Nexuspag envia campos na raiz do body (não em body.data)
    const paymentId = body.transaction_id || body.txid || body.external_id || (body.data && (body.data.id || body.data.transaction_id));
    ...
    return {
      ...
      payerEmail: null,
      payerName: body.payer_name || (body.data && body.data.payer_name) || null,
    };
  },
  asaas: function(body) {
  ...
  omegapay: function(body) {
    if (body.event !== "TRANSACTION_PAID") return null;
    const t = body.transaction;
    if (!t || t.status !== "COMPLETED") return null;
    return {
      paymentId: t.identifier || t.id,
      status: "approved",
      amount: t.amount,
      payerEmail: body.client && body.client.email,
      payerName: body.client && body.client.name,
    };
  },
...
function detectGateway(body, headers) {
  ...
  if (h) return h;
  const nexuspagHeader = headers && (headers.get ? headers.get("x-nexuspag-event") : headers["x-nexuspag-event"]);
  if (body.action && body.action.startsWith("payment")) return "mercadopago";
  if (body.event && body.event.startsWith("TRANSACTION_") && body.transaction) return "omegapay";
  if (body.event && body.event.startsWith("PAYMENT_")) return "asaas";
  if ((body.event === "payment.confirmed" || nexuspagHeader === "payment.confirmed") && (body.transaction_id || (body.data && body.data.id))) return "nexuspag";
